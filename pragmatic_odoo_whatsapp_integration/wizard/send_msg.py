...
class ScanWAQRCode(models.TransientModel):
    ...
    def _get_default_image(self):
        if self._context.get('active_model') == 'res.users' and self._context.get('active_id'):
            res_users_id = self.env['res.users'].sudo().search([('id', '=', self._context.get('active_id'))])
            url = res_users_id.whatsapp_endpoint + '/status?token=' + res_users_id.whatsapp_token
            response = requests.get(url)
            json_response = json.loads(response.text)
            if (response.status_code == 201 or response.status_code == 200) and (json_response['accountStatus'] == 'got qr code'):
                # qr_code_image
                qr_code_url = res_users_id.whatsapp_endpoint + '/qr_code?token=' + res_users_id.whatsapp_token
                response_qr_code = requests.get(qr_code_url)
                img = base64.b64encode(response_qr_code.content)
                res_users_id.write({'whatsapp_authenticate': True})
                return img
...
class SendWAMessageResPartner(models.TransientModel):
    ...
    def action_send_msg_res_partner(self):
        ...
        phone_numbers = []
        no_phone_partners = []
        status_url = self.env.user.whatsapp_endpoint + '/status?token=' + self.env.user.whatsapp_token
            ...
            if active_model == 'res.partner':
                for res_partner_id in self.partner_ids:
                    ...
                    if res_partner_id.country_id.phone_code and res_partner_id.mobile:
                        ...
                        phone_exists_url = self.env.user.whatsapp_endpoint + '/checkPhone?token=' + self.env.user.whatsapp_token + '&phone=' + str(res_partner_id.country_id.phone_code) + "" + whatsapp_msg_number_without_code
                            ...
                            if self.attachment_ids:
                                for attachment in self.attachment_ids:
                                    with open("/tmp/" + attachment.name, 'wb') as tmp:
                                        ...
                                        url_send_file = self.env.user.whatsapp_endpoint + '/sendFile?token=' + self.env.user.whatsapp_token
            ...
            if active_model == 'res.partner':
                for res_partner_id in self.partner_ids:
                    ...
                    if res_partner_id.country_id.phone_code and res_partner_id.mobile:
                        ...
                        whatsapp_msg_number_without_space = whatsapp_number.replace(" ", "")
                        whatsapp_msg_number_without_code = whatsapp_msg_number_without_space.replace('+' + str(res_partner_id.country_id.phone_code), "")
                        phone_exists_url = self.env.user.whatsapp_endpoint + '/checkPhone?token=' + self.env.user.whatsapp_token + '&phone=' + str(
