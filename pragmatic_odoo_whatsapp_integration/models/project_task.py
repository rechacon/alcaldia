...
class ProjectTask(models.Model):
        ...
        for user_id in self.user_ids:
            if user_id.partner_id.mobile:
                if user_id.partner_id.country_id.phone_code and user_id.partner_id.mobile:
                    ...
                    phone_exists_url = self.env.user.whatsapp_endpoint + '/checkPhone?token=' + self.env.user.whatsapp_token + '&phone=' + str(user_id.partner_id.country_id.phone_code) + "" + whatsapp_msg_number_without_code
                    ...
                    if (phone_exists_response.status_code == 200 or phone_exists_response.status_code == 201) and json_response_phone_exists['result'] == 'exists':
                        ...
                        if res_user_id.has_group('pragmatic_odoo_whatsapp_integration.group_project_enable_signature'):
                            ...
                            msg += "\n\n" + user_signature
                        url = self.env.user.whatsapp_endpoint + '/sendMessage?token=' + self.env.user.whatsapp_token
                        ...
                        if response.status_code == 201 or response.status_code == 200:
                            ...
                            project_task_user_rel_id = self.env['project.task.user.rel'].sudo().search([('user_id', '=', user_id.id), ('task_id', '=', self.id)])
                            project_task_user_rel_id.write({'whatsapp_msg_id': response_dict.get('id')})
    ...
    def _assigned_task_done(self):
        project_task_user_rel_ids = self.env['project.task.user.rel'].search([('whatsapp_msg_id', '!=', None)])
        Param = self.env['res.config.settings'].sudo().get_values()
        for project_task_user_id in project_task_user_rel_ids:
            res_partner_id = self.env['res.partner'].search([('id', '=', project_task_user_id.user_id.partner_id.id)])
            ...
            try:
                ...
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    for messages in response_dict['messages']:
                        current_whatsapp_msg_id = project_task_user_id.whatsapp_msg_id.partition("true_")[2].partition("_")[2]
                        if not messages['quotedMsgId'] == None and current_whatsapp_msg_id in messages['quotedMsgId']:
                            if messages['body'] == 'done' or messages['body'] == 'Done':
                                task_type_done_id = project_task_user_id.task_id.env['project.task.type'].search([('name', '=', 'Done')], limit=1)
                                if task_type_done_id:
                                    stage_id = project_task_user_id.task_id.write({'stage_id': task_type_done_id.id})
            except Exception as e_log:
